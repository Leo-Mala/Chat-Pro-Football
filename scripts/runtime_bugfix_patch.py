...
# Bulk persistence: Room list @Upsert/@Insert iterates prepared statements inside the existing
# transaction. 100-row caller chunking only multiplies DAO call overhead.
replace_once(
...
# Persisted fixture lookup for idempotent match-stat finalization.
replace_once(
...
# Seasonal reset preserves cumulative career stats and clears the actual seasonal columns.
replace_once(
    "app/src/main/java/com/example/data/daos.kt",
...
# Auto-simulated user fixture: finalize + stats exactly once from persisted state.
replace_once(
...
# Live fixture: same persisted guard.
replace_once(
...
# Maintain cumulative and seasonal player counters together.
replace_once(
    "app/src/main/java/com/example/ui/viewmodel/GameViewModelMatch.kt",
...
# Player editor closes only after player + affected club strength transaction commits.
replace_once(
...
# Real new-career phase instrumentation.
replace_once(
    "app/src/main/java/com/example/ui/viewmodel/GameViewModel.kt",
...
print("runtime bugfix patch: all exact replacements applied")
