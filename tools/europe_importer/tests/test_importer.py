...
class ImporterPipelineTest(unittest.TestCase):
    ...
    def test_full_premier_league_fixture_generates_valid_canonical_dataset(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(20, result.manifest["clubCount"])
            self.assertEqual(361, result.manifest["playerCount"])
            self.assertEqual(1, result.manifest["loanCount"])
            self.assertEqual("FIXTURE_ONLY", result.manifest["validationStatus"])
    ...
    def test_duplicate_player_and_missing_position_groups_are_rejected(self):
        raw = build_premier_league_api_fixture()
        normalized = normalize(raw, REQUEST)
        club0 = normalized["teams"][0]
        duplicate = copy.deepcopy(club0["players"][0])
