...
def _has_day_precision_birth(entity: dict[str, Any]) -> bool:
    for claim in (entity.get("claims") or {}).get("P569", []):
        ...
        if not isinstance(value, dict) or not value.get("time"):
            continue
        try:
            if int(value.get("precision") or 0) >= 11:
                return True
        except (TypeError, ValueError):
            continue
    return False
...
def install_p1532_discovery_bridge(provider: Any, overrides_path: Path) -> None:
    ...
    * fill P569 when no usable day-precision birth date exists and an official ``birthDates``
      override resolves to exactly one QID.
    ...
    def entities(qids: list[str]) -> dict[str, dict[str, Any]]:
        ...
        for qid, original_entity in source.items():
            ...
            if verified_birth and not _has_day_precision_birth(entity):
                if entity is original_entity:
                    entity = copy.deepcopy(entity)
