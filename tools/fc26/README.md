...
## Validated current-game coverage

The checked-in `reports/fc26_club_mapping_report.json` is generated by the full Room integration test against the current Pro Football universe. Current validated coverage is:

- 18,405 source players and 662 source clubs;
- 139 source clubs safely `MATCHED`;
- 511 `UNMATCHED` and 12 `AMBIGUOUS` clubs left unresolved instead of guessed;
- 3,890 players imported through matched clubs plus 89 FC26 free agents = 3,979 real players imported;
- 14,426 FC26 players skipped because their source club is not safely mapped yet;
- 2,405 Pro Football clubs retain procedural fallback rosters;
- all 1,325 source loan markers remain unresolved as `PlayerLoan` reconstruction because the snapshot does not provide safe loan duration data.

The report also records the exact per-club decisions, source distributions, Room persistence/reload proof and CI-run performance measurements.
...
- Calendar generation only registers a lazy seed request; the 18,405-player asset is read when the exact initial new-save `saveTeams -> savePlayers` persistence sequence begins. Season transitions and unrelated later persistence cannot consume a stale seed request.
